...
export const en = {
  ...
  autofill_multi_jc: 'This site has {count} job codes. {job_code} matches the day you are settling — open the Job Code cell to pick another.',
  ...
  sitejc_subtitle: 'The lookup that fills in a Job Code and its period when a Site ID is typed. A site has one row per task raised against it.',
  col_site_id: 'Site ID',
  col_job_code: 'Job Code',
  col_task_date: 'Task Date',
  col_site_jc: 'Site ID-JC',
  col_period: 'Period',
  sitejc_add: 'Add site',
  sitejc_edit: 'Edit site',
  sitejc_upload: 'Upload Excel',
  sitejc_search_placeholder: 'Search Site ID or Job Code…',
  sitejc_id_locked: 'The Site ID and Job Code together are the key and cannot be changed. To correct either, delete this row and add the right one.',
  sitejc_period_hint: 'Set from the Task Date. Changing it here holds until the next upload, which sets it from the date again.',
  sitejc_task_date_hint: 'The date the task was raised. It decides the period, and it is how the grid picks between a site’s job codes.',
  sitejc_multi_hint: 'This site has {count} job codes. The grid picks the one whose task date fits the day being settled.',
  ...
  sitejc_jc_unsplittable: '“{value}” has no Job Code after a hyphen — expected something like K4429-ABD01.',
  sitejc_single_jc_only: 'One Job Code per row — a “/” belongs in an entry, not in the lookup.',
  sitejc_too_long: 'Longer than {max} characters.',
  sitejc_saved: 'Site saved.',
  sitejc_deleted: 'Removed from the lookup.',
  sitejc_period_changed: '{site} moved to a different period.',
  sitejc_delete_title: 'Remove this job code?',
  sitejc_delete_text: '“{site}” will no longer offer “{job_code}” when a coordinator types it. Any other job codes on the site stay.',
  ...
  sitejc_empty_text: 'Upload the Site ID-JC tracking file, or add rows one at a time. Every row needs a Site ID, a Job Code and a task date.',
  ...
  import_effect_title: 'What this changes',
  import_stat_added: 'New',
  import_stat_changed: 'Changed',
  import_stat_unchanged: 'Unchanged',
  import_stat_removed: 'Removed',
  import_confirm: 'Import',
  import_confirm_skipping: 'Import {ready}, skip {skipped}',
  import_none_usable: 'None of the rows in this file could be read. Check that the Site ID-JC column holds values like K4429-ABD01.',
  import_replace_unknown: 'The file replaces the lookup, but the current lookup could not be read, so there is no telling how many rows that removes. Reload the screen before importing.',
  import_replace_note: 'The file replaces the lookup. Nothing currently in it is missing from the file, so nothing is removed.',
  import_replace_warning: 'The file replaces the lookup: {removed} rows are not in it and will be removed. Entries already recorded are unaffected.',
  import_period_note: 'Old and new are set from the Task Date — the file’s own Old/New column is not read.',
  import_no_task_date_column: 'This file has no Task Date column. Every row will be imported as new, and the grid will have no date to pick a job code by.',
  import_undated_note: '{count} rows have no readable task date. They will be imported as new.',
  import_duplicates_note: 'The file listed the same Site ID and Job Code more than once. They are matched ignoring case, so the last row for each wins.',
  ...
  import_skipped_title: 'Rows that will be skipped',
  import_skipped_note: 'These {count} rows cannot be read and will not be imported. Everything else in the file is. Fix them at source and they will come in with the next export.',
  import_problems_more: 'and {count} more.',
  import_row_label: 'Row {row}',
  import_done: '{created} added, {updated} updated, {removed} removed.',
