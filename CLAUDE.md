...
4. **Deletion is status, not row removal.** The one exception is a row that is in the coordinator's own hands — `draft`, or `returned` and therefore on its way back to draft (6.1) — and, by extension, a whole settlement holding nothing but such rows (`delete_settlement`, 3.5). Anything sitting with a manager (`confirmed`, `approved`) or already in a finance file (`exported`) is never hard-deleted — it is returned, or it stays with its status, and it blocks the deletion of the settlement around it.
...
| `persite_downloaded_at` | ISO datetime | Last time this batch's per-site file was downloaded. Server-set by `record_persite_download` |
| `persite_downloaded_by` | user_id | Who downloaded it last. Server-set |
| `persite_download_count` | integer | How many times in all. Blank reads as 0 |

The three `persite_*` columns are **appended on first use** (`ensureColumns`), and a batch exported before they existed reads as not downloaded.
...
## 3.5 Coordinator — own sheet only (9)
...
- **`export_batch_rows`** — `{batch_ids}` (or the older single `batch_id`; at most 50) → the entries stamped with any of those `export_batch_id`s, gathered in **one** sweep of the coordinator sheets, plus each batch's own log row (`batches`, in the order asked; `batch` is the first). An id with no log row refuses the whole call. Read-only; writes nothing and can claim nothing. This is what the **per-site** file is built from (7.1), and it selects on the batch id rather than on the team-period predicate so the per-site breakdown divides exactly the lines the finance file carried — a predicate re-run later can return a different set (a row approved since, a second batch for the same team under rule 9).
- **`record_persite_download`** — `{batch_ids}` → stamps `persite_downloaded_at` / `_by` and increments `persite_download_count` on those ExportLog rows, under the script lock. Called by the export screen **after** the file has been handed to the browser — kept apart from `export_batch_rows` so that a read stays a read, and a build that fails is never recorded as a download. It touches those three columns and nothing else.
...
**Confirm counts `draft` *and* `returned` rows.** A returned row is back in the coordinator's hands (6.1), and `runConfirm` saves both grids before it calls `confirm_track` — so a returned row he has fixed is already a `draft` by the time the server decides, and moves with its track. Counting only `draft` in the client's readiness check was a dead end: the coordinator filled in the very thing the row was returned for, was told "nothing to confirm", and pressing the button refused without even saving. A returned row he has *not* edited stays `returned` server-side and simply does not move.
...
**Several batches make one per-site file.** Each log row has a tick box (with select-all in the header); **Per-site file for *n* selected** builds a single workbook with **one table** holding every ticked batch, batch by batch, oldest first — not a tab per batch, because the file is a register to pivot and every row already names its team, Tracking#, coordinator and date. It is named `Per Site — 4 batches — 2026-09-10.xlsx`. A one-batch file keeps its existing name.

**The log says whether a batch's per-site file has been downloaded** — "Not downloaded", or when, by whom and how many times — recorded server-side (`record_persite_download`) so every manager sees the same answer, not just the one at that desk. **Select not downloaded** ticks exactly the batches still waiting. Downloading one that already went is never refused (finance loses files), but the screen **asks first**, naming each such batch and who last took it. "Downloaded" means the app produced the file; whether the browser saved it is beyond what a page can know.
