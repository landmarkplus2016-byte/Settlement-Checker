...
4. **Deletion is status, not row removal.** The one exception is a row that is in the coordinator's own hands — `draft`, or `returned` and therefore on its way back to draft (6.1). Anything sitting with a manager (`confirmed`, `approved`) or already in a finance file (`exported`) is never hard-deleted — it is returned, or it stays with its status.
...
- Never hard-delete anything except a `draft` or `returned` entry via `delete_entry` / `delete_entries`. A `confirmed` or `approved` row is with a manager and is returned, never deleted; an `exported` one is locked.
